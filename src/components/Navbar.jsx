    ...
    return (
        <header className={`fixed inset-x-0 top-0 z-50 px-4 sm:px-6 lg:px-10 transition-all ${scrolled ? 'py-3' : 'py-5'}`}>
            <nav className={`mx-auto flex max-w-360 items-center justify-between rounded-full border border-black/10 px-4 py-2.5 backdrop-blur-xl transition ${scrolled ? 'bg-paper/85 shadow-sm' : 'bg-paper/55'}`}>
                <a href="#top" className="font-mono text-sm font-medium tracking-tight">TSIZEHENA<span className="ml-1 text-black/50">S</span></a>

                <div className="hidden items-center gap-7 sm:flex">
                    {links.map(
                        ([label, href]) => <a key={href} className="font-mono text-[12px] uppercase tracking-[.12em] text-black/70 transition hover:text-black" href={href}>{label}</a>
                    )}
                </div>
